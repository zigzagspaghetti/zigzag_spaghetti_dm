...
from zigzag.zigzag_sp import zigzag_sp_function
...
def load_graph_classification_dataset(dataset_name, deg4feat=False, PE=True):
    ...
    final_dataset = []
    for g, y in dataset:
        zigzag_sp_feat_1 = zigzag_sp_function((g.adj()).to_dense(), g.ndata["attr"], scaleParameter=0.05,
                                              beta_schedule='linear', beta_1=0.000335, beta_T=0.03379,
                                              T=50)  # 2 * (T-1)
        zigzag_sp_feat_2 = zigzag_sp_function((g.adj()).to_dense(), g.ndata["attr"], scaleParameter=0.1,
                                              beta_schedule='linear', beta_1=0.000335, beta_T=0.03379,
                                              T=50)  # 2 * (T-1)
        zigzag_sp_feat_3 = zigzag_sp_function((g.adj()).to_dense(), g.ndata["attr"], scaleParameter=0.2,
                                              beta_schedule='linear', beta_1=0.000335, beta_T=0.03379,
                                              T=50)  # 2 * (T-1)
        zigzag_sp_feat = (zigzag_sp_feat_1 + zigzag_sp_feat_2 + zigzag_sp_feat_3) / 3
        final_dataset.append((g.remove_self_loop().add_self_loop(), y, zigzag_sp_feat))



    print(f"******** # Num Graphs: {len(final_dataset)}, # Num Feat: {feature_dim}, # Num Classes: {num_classes} ********")

    return final_dataset, (feature_dim, num_classes)
