...
def pretrain(model, train_loader, optimizer, device, epoch, logger=None):
    ...
    for batch in train_loader:
        #print("what batch consists of:", batch)
        batch_g, _, zigzag_sp_feat = batch # done
        batch_g = batch_g.to(device)
        feat = batch_g.ndata["attr"]
        loss, loss_dict = model(batch_g, feat, zigzag_sp_feat)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def collate_fn(batch):
    ...
    zigzag_feat = [x[2] for x in batch]
    batch_g = dgl.batch(graphs)
    labels = torch.cat(labels, dim=0)
    zigzag_feats = torch.cat(zigzag_feat, dim=0)
    return batch_g, labels, zigzag_feats
    ...
    for i, seed in enumerate(cfg.seeds):
        ...
        if args.resume:
            if osp.isfile(cfg.pretrain_checkpoint_dir):
                ...
                checkpoint = torch.load(cfg.checkpoint_dir, map_location=torch.device('cuda:0'))
